            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpMap->KeyFramesInMap()>2)
                {
                    if(!mpCurrentKeyFrame->mbVINSInitedKF)
                    {
                        ...
                        cout<<"(LocalMapping Thread):"<<mpCurrentKeyFrame->mnId<<" Visual BA END"<<endl;
                    }
                    else
                    {
...
void LocalMapping::RemovefromLocalWindow()
{
    for(list<KeyFrame*>::iterator it = lLocalKeyFrames.begin(); it != lLocalKeyFrames.end(); it++)
    {
        KeyFrame* pKF = *it;
        if(pKF->isBad())
